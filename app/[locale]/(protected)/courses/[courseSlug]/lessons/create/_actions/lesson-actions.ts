...
export async function createLesson(formData: FormData) {
  ...
  if (newModuleTitle) {
    ...
    moduleRecord = await prisma.courseModule.create({
      data: { title: newModuleTitle, slug: moduleSlug, order: moduleOrder, courseId: course.id },
    });
  } else {
  ...
  if (newChapterTitle) {
    ...
    const lastChapter = await prisma.courseChapter.findFirst({
    ...
    let suffixChap = 1;
    while (
      await prisma.courseChapter.findUnique({
        where: { moduleId_slug: { moduleId: moduleRecord.id, slug: chapterSlug } },
      })
    ) {
