...
export async function createNotification(request: NotificationCreateRequest): Promise<string> {
  ...
  const effectiveChannels = request.channels || getEffectiveChannels(
    request.type, 
    preferences, 
  ...
  const message = request.customMessage || replaceTemplateVariables(template.message, request.metadata)
  ...
  const notification = await db.notification.create({
    data: {
      ...
      relatedEntityType: request.metadata.questionId ? 'Question' : 
                        request.metadata.answerId ? 'Answer' :
                        request.metadata.courseId ? 'Course' :
                        request.metadata.lessonId ? 'Lesson' : undefined,
      relatedEntityId: request.metadata.questionId || 
                      request.metadata.answerId || 
                      request.metadata.courseId || 
                      request.metadata.lessonId || undefined
    }
  })
...
export async function createNotifications(requests: NotificationCreateRequest[]): Promise<string[]> {
